...
i.default.setSettings(e)},onMessage:function(e){var t=e.origin||e.originalEvent.origin,n=d.default.getConfig(),g=n&&n.cssNames
...
break
case"notify_frame":var I=w.data,k=document.getElementById(s.default.frameDivId),E=g&&g.open||"fc-open"
"expand"===I?(k.removeAttribute("style"),k.classList.add("h-open-notify"),k.classList.add(E),k.classList.add(I)):"close"===I?(k.removeAttribute("style"),k.classList.remove("h-open-notify"),k.classList.remove(E),k.classList.remove("expand")):"mobile-view"===I?k.classList.add("fc-mobile-view"):(k.classList.add("h-open-notify"),k.classList.add(E))
break
case"resize_frame":var C=document.getElementsByTagName("BODY")[0],x=g&&g.open||"fc-open"
r.default.opened(w.isOpen),r.default.isOpened()?(_.classList.add("h-open-container"),_.classList.add(x),_.classList.add("widget-open-animate"),o.default.publish("widget:opened"),setTimeout(function(){v(this,this),_.classList.remove("widget-open-animate")}.bind(this),1e3),C&&C.classList.add("fc-widget-open")):(_.removeAttribute("style"),_.classList.remove("h-open-container"),_.classList.remove(x),o.default.publish("widget:closed"),C&&C.classList.remove("fc-widget-open"))
break
...
break
case"frame_state_change":var L=document.getElementById(s.default.frameDivId),M=w&&w.data,O=M&&M.frameState,S=i.default.getSettings()&&i.default.getSettings().enabledFeatures
o.default.publish("frame:statechange",w),"initialized"===O&&(i.default.isJWTEnabled()&&i.default.isJWTStrictMode()&&L.classList.add("fc_dn"),this.loadWidget(),S&&-1!==S.indexOf("COBROWSING")&&m.default.init({host:i.default.getHost(),locale:d.default.getProperties().locale})),"loaded"===O&&i.default.loaded(!0),"authenticated"!==O&&"not_authenticated"!==O||i.default.isJWTEnabled()&&i.default.isJWTStrictMode()&&(w.success?L.classList.remove("fc_dn"):304!==w.status&&this.unloadWidget())
break
...
case"remove_storage_item":if(a.default.storageAvailable){var D=w.data
D&&D.key&&localStorage.removeItem(D.key)}break
case"startScreenShare":window.fc_cobrowse&&window.fc_cobrowse.allowScreenShare(e)
break
case"set_lsds_item":if(a.default.storageAvailable){var B=w.data,F=B&&B.key
if(F){var V=B&&B.value
